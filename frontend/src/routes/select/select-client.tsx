...
import { logoutStudent } from "@/lib/auth-client"
    ...
    <AppPageShell title="講者志願選填" backHref="/" backLabel="回到首頁">
      <Alert className="grid-cols-[auto_1fr_auto] items-center gap-x-3 has-[>svg]:grid-cols-[auto_1fr_auto] [&>svg]:translate-y-0">
        ...
        <Button
          ...
          onClick={() => void logoutStudent("/select")}
        >
...
function SpeakerRankingPanel({
  ...
  const speakerNames = useMemo(
    () => speakers.map((speaker) => speaker.speakerName),
    [speakers],
  )
    ...
    <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto">
      ...
      <div className="flex flex-col gap-4">
        <ItemGroup className="gap-3">
          {speakers.map((speaker, index) => {
            const isActive = activeDrag?.speakerName === speaker.speakerName
            ...
            return (
              <SpeakerRankItem
                ...
                onMoveUp={() => moveSpeaker(speaker.speakerName, -1)}
                onMoveDown={() => moveSpeaker(speaker.speakerName, 1)}
                onDragHandlePointerDown={(event) =>
                  handleDragStart(speaker.speakerName, event)
                }
                onDragHandlePointerMove={(event) =>
                  handleDragMove(speaker.speakerName, event)
                }
                onDragHandlePointerUp={(event) =>
                  finishDrag(speaker.speakerName, event)
                }
                onDragHandlePointerCancel={(event) =>
                  finishDrag(speaker.speakerName, event)
                }
                onDragHandleLostPointerCapture={() =>
                  handleLostPointerCapture(speaker.speakerName)
                }
              />
            )
          })}
            ...
            <AlertDescription>
              <p>
                {speakers
                  .map(
                    (speaker, index) =>
                      `${rankLabel(index + 1)}：${speaker.speakerName}`,
                  )
                  .join(" / ")}
              </p>
            </AlertDescription>
...
function SpeakerRankItem({
  ...
  return (
    <Item
      ...
      variant="outline"
      tabIndex={0}
      aria-label={`${speaker.speakerName}，目前${rankLabel(rank)}`}
    ...
    >
      ...
      >
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          aria-label={`上移 ${speaker.speakerName}`}
          disabled={!canMoveUp}
          onClick={onMoveUp}
        >
        ...
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          aria-label={`下移 ${speaker.speakerName}`}
      ...
      <ItemActions>
        <Button
          ...
          variant="outline"
          size="icon-sm"
          aria-label={`拖曳 ${speaker.speakerName}`}
...
function SpeakerAvatar({ speaker, className }: SpeakerAvatarProps) {
  return (
    <Avatar size="lg" className={className}>
      <AvatarImage src={speaker.avatar} alt={speaker.speakerName} />
      <AvatarFallback>{speakerInitials(speaker.speakerName)}</AvatarFallback>
    </Avatar>
  )
}
